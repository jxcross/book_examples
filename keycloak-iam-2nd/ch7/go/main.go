...
const KEYCLOAK_REALM_URL = "http://localhost:58081/realms/myrealm"
...
func createConfig(provider oidc.Provider) (oidc.Config, oauth2.Config) {
	...
	config := oauth2.Config{
		...
		ClientSecret: "6ve1B4Z3tzoJsSiC5OCaNQZlFWHLlVRC", // change CLIENT_SECRET with the secret generated by Keycloak for the mywebapp client. This option is a string.
		Endpoint:     provider.Endpoint(),
		RedirectURL:  "http://localhost:58080/auth/callback",
...
func main() {
	...
	log.Printf("To authenticate go to http://%s/", "localhost:58080")
	log.Fatal(http.ListenAndServe("localhost:58080", nil))
}
...
func redirectHandler(resp http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		http.Redirect(resp, req, "http://localhost:58080", http.StatusFound)
	} else {
		// user authenticated, landing page
		io.WriteString(resp, "<html><body>"+
			"You are already authenticated. "+
			"Click <a href='"+KEYCLOAK_REALM_URL+"/protocol/openid-connect/logout?client_id="+oidcConfig.ClientID+"&post_logout_redirect_uri=http://localhost:58080/logout'>here</a> to logout"+
			"</body></html>")
	}
}
