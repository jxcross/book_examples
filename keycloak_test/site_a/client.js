...
function generateAuthenticationRequest() {
    ...
    setOutput('output-authenticationRequest', req.replace('?', '<br/><br/>').replaceAll('&', '<br/>') + '<hr/>' + req);
...
function init() {
    ...
    if (code) {
        ...
        setOutput('output-authenticationResponse', 'requestURL='+window.location.href+'<hr/>'+'code=' + code);
    }
