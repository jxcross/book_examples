...
from agents.twitter_lookup_agent import lookup as twitter_lookup_agnet
...
if __name__ == "__main__":
    ...
    #----------------------------------
    # Agent: Linkedin 검색해서 URL 가져오기
    #----------------------------------
    linkedin_profile_url = linkedin_lookup_agent(name=name)
    twitter_username = twitter_lookup_agnet(name=name)
    ...
    summary_prompt_template = PromptTemplate(
        input_variables=["linkedin_information", "twitter_infomration"], template=summary_template
    )
    ...
    tweets = scrape_user_tweets(username=twitter_username, num_tweets=5) # num_tweets=100
